...
#
# SDK codegen hooks: this Makefile `-include`s an optional, provider-owned
# `sdk-hooks.mk` (see below) and invokes `PRE_GEN_SDK_<LANG>` /
# `POST_GEN_SDK_<LANG>` slots around each per-language SDK codegen recipe.
# Use these to fine-tune `pulumi package gen-sdk` output without forking this
...
compare_sdks: compare_sdk_dotnet compare_sdk_go compare_sdk_nodejs compare_sdk_python
...
# the provider repo root it is included here and may define `PRE_GEN_SDK_<LANG>`
# / `POST_GEN_SDK_<LANG>` (LANG in GO, NODEJS, PYTHON, DOTNET, JAVA), which are
# invoked around the matching per-language codegen recipe above. This file is
# opt-in and is never created by ci-mgmt. See `sdk-hooks.mk.example` in
# pulumi/ci-mgmt for the contract and a worked example.
-include sdk-hooks.mk
